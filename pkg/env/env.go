	...
	for i := 0; i < rv.NumField(); i++ {
		...
		if !valueField.CanSet() {
			return ErrUnsupportedField{
				Value: fmt.Sprintf("field %s.%s is not exported", t.Name(), typeField.Name),
			}
		}
...
func set(t reflect.Type, f reflect.Value, value string) error {
	...
	default:
		return ErrUnsupportedField{
			Value: fmt.Sprintf("field %s has unsupported type %s", t.Name(), t.Kind()),
		}
	}
...
func parseTag(tagString string) tag {
	...
	for _, key := range envKeys {
		...
		} else if strings.ToLower(key) == "required" {
			t.Required = true
		} else {
